                ...
                <h4 class="mb-4 fw-bold">Editar Médico</h4>
                    <form method="POST" action="{{ route('admin.medicos.update', $medico->id) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $medico->name) }}" required>
                            @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        <div class="mb-4">
                            <label class="form-label">Email</label>
                            <input type="email" name="email" class="form-control @error('email') is-invalid @enderror" value="{{ old('email', $medico->email) }}" required>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="telefono" class="form-control @error('telefono') is-invalid @enderror" value="{{ old('telefono', $medico->telefono) }}">
                            @error('telefono') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        <div class="mb-4">
                            <label class="form-label">Cédula profesional</label>
                            <input type="text" name="cedula_profesional" class="form-control @error('cedula_profesional') is-invalid @enderror" value="{{ old('cedula_profesional', $medico->medicoPerfil->cedula_profesional ?? '') }}">
                            @error('cedula_profesional') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        <div class="mb-4">
                            <label class="form-label">Universidad</label>
                            <input type="text" name="universidad" class="form-control @error('universidad') is-invalid @enderror" value="{{ old('universidad', $medico->medicoPerfil->universidad ?? '') }}">
                            @error('universidad') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        <div class="mb-4">
                            <label class="form-label">Años de experiencia</label>
                            <input type="number" name="experiencia_anios" class="form-control @error('experiencia_anios') is-invalid @enderror" value="{{ old('experiencia_anios', $medico->medicoPerfil->experiencia_anios ?? '') }}" min="0">
                        ...
                        <div class="d-flex justify-content-between">
                            <a href="{{ route('admin.medicos') }}" class="btn btn-outline-secondary"><i class="fa-regular fa-xmark me-1"></i>Cancelar</a>
