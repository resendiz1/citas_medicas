...
<div class="d-flex flex-wrap gap-1 mb-1">
    @if ($cita->estado === 'pendiente')
        <form action="{{ route('citas.estado', $cita->id) }}" method="POST" class="d-inline">
            ...
            <button class="btn btn-success btn-sm" style="font-size:0.65rem"><i class="fa-regular fa-check-circle me-1"></i>Confirmar</button>
        </form>
        ...
        <form action="{{ route('citas.estado', $cita->id) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Cancelar esta cita?')">
            ...
            <button class="btn btn-danger btn-sm" style="font-size:0.65rem"><i class="fa-regular fa-circle-xmark me-1"></i>Cancelar</button>
        </form>
    ...
    @elseif ($cita->estado === 'confirmada')
        ...
        <form action="{{ route('citas.estado', $cita->id) }}" method="POST" class="d-inline">
            ...
            <button class="btn btn-warning btn-sm" style="font-size:0.65rem"><i class="fa-regular fa-clock me-1"></i>En espera</button>
        </form>
        @else
        ...
        <form action="{{ route('citas.estado', $cita->id) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Marcar como no asistió?')">
            ...
            <button class="btn btn-danger btn-sm" style="font-size:0.65rem"><i class="fa-regular fa-user-slash me-1"></i>No asistió</button>
        </form>
        @else
            {!! $ds('No asistió', 'Solo disponible el día de la cita') !!}
        @endif
    @elseif ($cita->estado === 'en_espera')
        <a href="{{ route('consulta-medica.create', $cita->id) }}" class="btn btn-primary btn-sm" style="font-size:0.65rem"><i class="fa-solid fa-stethoscope me-1"></i>En consulta</a>
        <form action="{{ route('citas.estado', $cita->id) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Cancelar esta cita?')">
            @csrf @method('PUT')
            <input type="hidden" name="estado" value="cancelada">
            <button class="btn btn-danger btn-sm" style="font-size:0.65rem"><i class="fa-regular fa-circle-xmark me-1"></i>Cancelar</button>
    ...
    @elseif ($cita->estado === 'en_consulta')
        <form action="{{ route('citas.estado', $cita->id) }}" method="POST" class="d-inline">
            ...
            <button class="btn btn-secondary btn-sm" style="font-size:0.65rem"><i class="fa-regular fa-check-double me-1"></i>Finalizar</button>
        </form>
...
<div class="d-flex flex-wrap gap-1">
    <a href="{{ route('medico.paciente.show', $cita->paciente->id) }}" class="btn btn-outline-secondary btn-sm" style="font-size:0.65rem"><i class="fa-regular fa-user me-1"></i>Perfil</a>
    <a href="{{ route('citas.show', $cita->id) }}" class="btn btn-primary btn-sm" style="font-size:0.65rem"><i class="fa-regular fa-comment-dots me-1"></i>Chat</a>
    @if (!in_array($cita->estado, ['cancelada', 'no_asistio']))
    @if ($cita->consultaMedica)
        <a href="{{ route('consulta-medica.show', $cita->id) }}" class="btn btn-success btn-sm" style="font-size:0.65rem"><i class="fa-solid fa-stethoscope me-1"></i>Consulta</a>
    @elseif ($cita->estado === 'confirmada')
        <a href="{{ route('consulta-medica.create', $cita->id) }}" class="btn btn-primary btn-sm" style="font-size:0.65rem"><i class="fa-solid fa-plus me-1"></i>+ Consulta</a>
    @else
        {!! $ds('+ Consulta', 'Primero confirma la cita') !!}
    @endif
    @if ($cita->ultimaReceta)
        <a href="{{ route('recetas.show', $cita->ultimaReceta->id) }}" class="btn btn-warning btn-sm" style="font-size:0.65rem"><i class="fa-solid fa-prescription me-1"></i>Receta</a>
    @else
        @php $esHoy = $cita->fecha_hora->isToday(); @endphp
        @if ($esHoy)
        <a href="{{ route('recetas.create', $cita->id) }}" class="btn btn-primary btn-sm" style="font-size:0.65rem"><i class="fa-solid fa-plus me-1"></i>+ Receta</a>
        @else
...
<div class="d-flex flex-wrap gap-1">
    ...
    @elseif ($cita->estado === 'reprogramada' && $cita->fecha_reprogramada)
        ...
        <form action="{{ route('citas.reprogramacion.confirmar', $cita->id) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Confirmar la reprogramación para el {{ $cita->fecha_reprogramada->format('d/m/Y H:i') }}?')">
            @csrf
            <button class="btn btn-success btn-sm" style="font-size:0.65rem"><i class="fa-regular fa-circle-check me-1"></i>Aceptar</button>
        </form>
        <form action="{{ route('citas.reprogramacion.cancelar', $cita->id) }}" method="POST" class="d-inline" onsubmit="return confirm('¿Rechazar la reprogramación? La cita mantendrá su fecha original.')">
            @csrf
            <button class="btn btn-danger btn-sm" style="font-size:0.65rem"><i class="fa-regular fa-ban me-1"></i>Rechazar</button>
        </form>
    @endif
</div>
@else
<div class="d-flex flex-wrap gap-1" style="max-width:200px">
    ...
    @elseif ($cita->estado === 'en_espera')
        <form action="{{ route('citas.estado', $cita->id) }}" method="POST" class="d-inline">
            ...
            <button class="btn btn-primary btn-sm" style="font-size:0.65rem"><i class="fa-solid fa-stethoscope me-1"></i>En consulta</button>
        </form>
    @elseif ($cita->estado === 'en_consulta')
        <form action="{{ route('citas.estado', $cita->id) }}" method="POST" class="d-inline">
            @csrf @method('PUT')
