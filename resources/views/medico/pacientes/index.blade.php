...
<div class="container">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 class="fw-bold" style="color:#1266f1"><i class="fa fa-users me-2"></i>Mis Pacientes</h4>
        <a href="{{ route('medico.pacientes.create') }}" class="btn btn-primary neu-btn-sm"><i class="fa fa-plus me-1"></i>+ Nuevo paciente</a>
    </div>
    ...
    <form method="GET" class="mb-3">
        <div class="row g-2 align-items-center">
            ...
            <div class="col-auto">
                ...
                @if (request('search'))
                    <a href="{{ route('medico.pacientes.index') }}" class="btn btn-outline-secondary btn-sm"><i class="fa fa-rotate-left me-1"></i>Limpiar</a>
                @endif
            </div>
        </div>
    </form>
                ...
                <tbody>
                    @forelse ($pacientes as $paciente)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('medico.pacientes.edit', $paciente->id) }}" class="btn btn-outline-secondary btn-sm"><i class="fa fa-pen-to-square me-1"></i>Editar</a>
                                <form action="{{ route('medico.pacientes.destroy', $paciente->id) }}" method="POST" class="d-inline">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="btn btn-outline-secondary btn-sm" onclick="return confirm('¿Eliminar paciente?')"><i class="fa fa-trash-can me-1"></i>Eliminar</button>
                                </form>
                            </td>
                        </tr>
                    @empty
